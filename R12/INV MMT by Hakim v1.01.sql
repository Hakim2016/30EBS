...
mmt.expenditure_type,
       ...
       (SELECT gcc.concatenated_segments
         ...
         WHERE 1 = 1
           AND gcc.code_combination_id = mmt.distribution_account_id) acc,
       --mmt.source_code,
...
=======
   ...
   --AND mmt.pm_cost_collected = 'E'
   AND mtt.transaction_type_name = 'Project Material SOS'
   AND trunc(mmt.transaction_date) >= to_date('2019-01-25','yyyy-mm-dd')
   --AND mmt.transaction_id IN (82619011,84347932,84256067)
--70549802
--83220748--55059657--= 54868663--60911685--8915869
--AND mmt.last_update_date >= to_date('2018-12-06','yyyy-mm-dd')-- - 1 --SYSDATE - 2
--AND mmt.organization_id = 83--121--86
--AND mmt.inventory_item_id = 3939072
--AND mmt.transaction_id =5097--48624778-->= 
--= 38434933--54899211--54898548--54897910--56683536
...
--49368602 --2018
--AND mmt.created_by = 4270
--AND msi.segment1 = 'G0002204-C-0000'--'1000EX-EN'
/*AND EXISTS (SELECT 'Y'
 FROM mtl_sales_orders mso
WHERE 1 = 1
  AND mso.sales_order_id = mmt.transaction_source_id
...
SELECT *
 ...
 WHERE 1 = 1
   AND gcc.segment1 || '.' || gcc.segment2 || '.' || gcc.segment3 || '.' || gcc.segment4 || '.' || gcc.segment5 || '.' ||
       gcc.segment6 || '.' || gcc.segment7
      
       IN ('FB00.000.1161500990.1146011000.11001096.0.0'
       , 'FB00.000.1161500990.1146011000.11000948.0.0')
       ;

SELECT mmt.*
  FROM rcv_transactions mmt
 WHERE 1 = 1
   AND mmt.transaction_id = 5310173;
