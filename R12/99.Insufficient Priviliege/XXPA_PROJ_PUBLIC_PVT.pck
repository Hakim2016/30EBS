  ...
  * =============================================*/
  ...
  BEGIN
    SELECT PA_PROJECT_STRUCTURE_UTILS.GET_CURRENT_WORKING_VER_ID(P_PROJECT_ID)
      ...
      FROM DUAL;
      LOG('PUBLISHING L_STRUCTURE_VERSION_ID : '|| L_STRUCTURE_VERSION_ID);
    ...
    LOG('PUBLISHING X_RETURN_STATUS : '|| X_RETURN_STATUS);
     LOG('PUBLISHING X_MSG_DATA : '|| X_MSG_DATA);
    FOR I IN 1 .. NVL(X_MSG_COUNT, 0) LOOP
      PA_INTERFACE_UTILS_PUB.GET_MESSAGES(P_ENCODED       => 'F',
                                          P_MSG_INDEX     => I,
                                          P_MSG_COUNT     => X_MSG_COUNT,
    ...
    END LOOP;
    LOG('PUBLISHING X_RETURN_STATUS : '|| X_RETURN_STATUS);
    LOG('PUBLISHING X_MSG_COUNT : '|| X_MSG_COUNT);
    LOG('PUBLISHING X_MSG_DATA : '|| X_MSG_DATA);
  EXCEPTION
    WHEN OTHERS THEN
      LOG('PUBLISHING X_RETURN_STATUS : '|| X_RETURN_STATUS);
