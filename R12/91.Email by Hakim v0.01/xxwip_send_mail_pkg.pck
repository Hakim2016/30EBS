CREATE OR REPLACE PACKAGE xxwip_send_mail_pkg IS
  ...
  Description:
       Sent Email 
  History:
      1.00  2018/03/14 baobao.hu   Creation
  ==================================================*/
...
END xxwip_send_mail_pkg;
/
CREATE OR REPLACE PACKAGE BODY xxwip_send_mail_pkg IS
  ...
  g_pkg_name VARCHAR2(30) := 'XXWIP_SEND_MAIL_PKG';
  ...
  PROCEDURE init_global(x_return_status OUT NOCOPY VARCHAR2,
                        ...
                        p_receipt_list  OUT xxfnd_utl_smtp_helper.recipient_list,
                        p_smtp_host     OUT VARCHAR2,
                        p_from_email    OUT VARCHAR2) IS
    i                   NUMBER := 0;
  ...
  BEGIN
    ...
    FETCH cur_from_sender
      INTO p_smtp_host, p_from_email;
    CLOSE cur_from_sender;
    IF p_smtp_host IS NULL OR p_from_email IS NULL THEN
    ...
    FOR rec_receiver IN cur_to_receiver LOOP
      i := i + 1;
      p_receipt_list(i).recipient := rec_receiver.meaning;
      p_receipt_list(i).email_address := rec_receiver.description;
    END LOOP;
    /*  p_receipt_list(1).recipient := 'HBB';
    p_receipt_list(1).email_address := 'steven@hand-china.com';*/
    IF p_receipt_list.count = 0 THEN
      x_msg_data      := 'Lookup ' || l_email_lookup_type ||
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      log('Error in ' || g_pkg_name || '.' || l_api_name || ':' ||
  ...
  PROCEDURE generate_email_attachment(x_email_clob    IN OUT CLOB,
    ...
    x_email_content VARCHAR2(32767) := '<html xmlns:v="urn:schemas-microsoft-com:vml" 
             ...
             <head><meta http-equiv="content-type" content="text/html; charset=' ||
                                       g_encoding ||
                                       '"/> <style type="text/css">' ||
                                       l_style || '</style></head><body>' ||
                                       '<p align=center style="font-weight:bold;"><font size="4">' ||
                                       l_email_subject ||
                                       '</font></p><p align=left style="font-weight:bold;">Request Date:' ||
                                       to_char(SYSDATE, g_date_format) ||
                                       '</p><table>';
    l_header        VARCHAR2(32767) := '<tr><td align=left NOWRAP>MFG</td>' ||
                                       '<td align=left NOWRAP>Job</td>' ||
    ...
    CURSOR cur_data IS
      SELECT wmm.mfg_num,
       ...
       WHERE 1 = 1
         ...
         AND (((wro.required_quantity - wro.quantity_issued) > 0 AND
             wmm.attribute2 = 'FSHF') OR (wmm.attribute2 <> 'FSHF'))
       ...
       GROUP BY wmm.mfg_num,
                ...
                nvl(xxwip_wo_issue_pkg.get_onhand_qty(p_inventory_item_id => wro.inventory_item_id,
                    ...
                    0),
                wmm.attribute2
       ORDER BY mfg_num, job, attribute1;
  BEGIN
    SAVEPOINT l_save_point;
    x_return_status := fnd_api.g_ret_sts_success;
    x_email_content := x_email_content || l_header;
    dbms_lob.writeappend(lob_loc => x_email_clob,
                         amount  => length(x_email_content),
                         buffer  => x_email_content);
    FOR rec_data IN cur_data LOOP
      g_exist_data    := 1;
      x_email_content := '<tr><td align=left NOWRAP>' || rec_data.mfg_num ||
                         '</td>';
      x_email_content := x_email_content ||
                         '<td align=left style = "vnd.ms-excel.numberformat;@" NOWRAP>' ||
                         rec_data.job || '</td>';
      x_email_content := x_email_content || '<td align=left NOWRAP>' ||
                         rec_data.component_item_num || '</td>';
      x_email_content := x_email_content || '<td align=left NOWRAP>' ||
                         rec_data.job_quantity || '</td>';
      x_email_content := x_email_content || '<td align=left NOWRAP>' ||
                         rec_data.attribute1 || '</td>';
      x_email_content := x_email_content || '<td align=left NOWRAP>' ||
                         rec_data.open_quantity || '</td>';
      x_email_content := x_email_content || '<td align=left NOWRAP>' ||
                         rec_data.onhand_quantity_on_fshf || '</td>';
      x_email_content := x_email_content || '<td align=left NOWRAP>' ||
                         rec_data.onhand_quantity_on_frm || '</td>';
      x_email_content := x_email_content || '<td align=left NOWRAP>' ||
                         rec_data.supply_subinv || '</td></tr>';
      dbms_lob.writeappend(lob_loc => x_email_clob,
                           amount  => length(x_email_content),
                           buffer  => x_email_content);
    END LOOP;
  ...
  BEGIN
    ...
    successed := xxfnd_utl_smtp_helper.create_smtp_connection(p_smtp_hostname => p_smtp_host,
    ...
    ELSE
      xxfnd_conc_utl.log_msg('Mailer init failed(Outbound server name=' ||
                             p_smtp_host || ')');
    END IF;
  ...
  BEGIN
    ...
    IF g_send_flag = c_yes_flag AND g_exist_data = 1 THEN
      log('email sent£¡');
      FOR i IN 1 .. nvl(l_receipt_list.count, 0) LOOP
        log('receipter :');
        log(l_receipt_list(i).recipient || l_receipt_list(i).email_address);
      END LOOP;
      l_email_content := '<p align=left><font size ="4">' ||
                         'Please check the attachment for the detail information.' ||
                         '</font></p>';
