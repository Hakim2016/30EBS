         ...
         /*AND NOT EXISTS (SELECT 1 -- added by Jaron.li@2014-12-18 begin
               ...
               WHERE rt.po_line_id = pl.po_line_id(+)
                 ...
                 AND EXISTS (SELECT 1
                       ...
                       upper(gl.name) = 'HEA LEDGER'--g_hea_ledger --update by huangyan 20150108
...
/*         AND (l_budat_limit IS NULL AND --
             ...
             to_char(xah.accounting_date, g_time_format) = l_budat_limit)*/
            --
         --AND xal.accounting_date >= p_data_start_date
         /*AND NOT EXISTS (SELECT xad.unique_id
                FROM xxgl_acct_details_lines_hfg xad
               WHERE xad.source_table = l_source_table
        ...
        */         
        AND xah.ae_header_id IN (24357779,25309847)
